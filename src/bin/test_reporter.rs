...
//!
//! Usage:
//!   cargo run --bin test_reporter -- --output playground/static/test-results.json
...
fn load_expected_outputs() -> serde_json::Value {
    let expected_path =
        PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("playground/static/expected-outputs.json");

    if !expected_path.exists() {
        eprintln!(
            "Warning: expected-outputs.json not found. Run 'node scripts/generate-expected-outputs.mjs > playground/static/expected-outputs.json' first."
        );
