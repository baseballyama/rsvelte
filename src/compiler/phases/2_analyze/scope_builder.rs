...
mod tests {
    use super::*;
    use crate::compiler::phases::phase1_parse::ParseOptions;
    use crate::parse;

    #[test]
    fn test_event_handler_assignment_detection() {
        let source = r#"<script>
    let foo = false;
    let bar = [false];
</script>
...
"#;
        ...
        let ast = parse(source, options).expect("Failed to parse");
        let (scope_root, _errors) = build_scopes(&ast, source, false);

        // Print bindings for debugging
        println!("Bindings found:");
        for binding in &scope_root.bindings {
            println!(
                "  {} - kind={:?}, reassigned={}, mutated={}",
                binding.name, binding.kind, binding.reassigned, binding.mutated
            );
        }

        // Find foo binding
        let foo_binding = scope_root.bindings.iter().find(|b| b.name == "foo");
        assert!(foo_binding.is_some(), "foo binding should exist");
        let foo = foo_binding.unwrap();
        assert!(foo.reassigned, "foo should be marked as reassigned");

        // Find bar binding
        let bar_binding = scope_root.bindings.iter().find(|b| b.name == "bar");
        assert!(bar_binding.is_some(), "bar binding should exist");
        let bar = bar_binding.unwrap();
        assert!(bar.mutated, "bar should be marked as mutated");
    }
}
