...
use crate::compiler::phases::phase3_transform::shared::js_scan::{
    after_keyword, after_keywords, contains_identifier, find_code, skip_opaque,
};
...
fn separate_same_line_legacy_export_declarations<'a>(
    ...
    if !contains_identifier(script, "export") {
    ...
    for pair in parsed.program.body.windows(2) {
        ...
        let previous_head = previous_text.trim_start();
        if (after_keywords(previous_head, &["export", "let"]).is_some()
            || after_keywords(previous_head, &["export", "var"]).is_some())
            && !gap.contains('\n')
