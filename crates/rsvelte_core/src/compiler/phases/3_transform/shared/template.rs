...
///
/// Mirrors upstream `is_void`'s `VOID_ELEMENT_NAMES` (svelte/src/utils.js),
/// which includes `command` and `keygen`. Used by both the client template
/// printer and the server, so they agree on self-closing output.
