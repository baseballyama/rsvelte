...
const TABLE: &[EarlyError] = &[
    ...
    // OXC spells the two `arguments` contexts with one message each, and acorn
    // words them differently from OXC and from each other — "class static
    // initialization block" against OXC's "static initialization block" — so
    ...
    // acorn words this one identically to OXC, but the needle keys on the head
    // rather than the tail: "is not defined" alone would adopt any future OXC
    // diagnostic ending that way and report it as an export error.
    EarlyError {
        needle: "Export '",
        at: At::First,
