...
/// must not swallow a real diagnostic. Every cell was generated by running the
/// input through `svelte.compile`, which is why `{f(}` — an expression that
/// runs out of input — reads `Unexpected token` and not OXC's own wording.
#[test]
fn a_body_with_code_in_it_keeps_its_own_message() {
    for (src, code, message, at) in [
        ("{1 +}", "js_parse_error", "Unexpected token", 4),
        ("{f(}", "js_parse_error", "Unexpected token", 3),
        ("{42 = nope}", "js_parse_error", "Assigning to rvalue", 1),
        (
            "{'a' /* c */ 'b'}",
            "expected_token",
            "Expected token }",
        ...
        assert_eq!(
            actual,
            (code.to_string(), message.to_string(), at, at),
            "for {src:?}"
        );
    }
}
