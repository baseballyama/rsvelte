...
#[test]
fn a_comment_before_the_declaration_still_collapses_the_trailing_one() {
    let oracle = vec![
        "let v = $.fallback($$props['v'], 1 // c".to_string(),
        ");".to_string(),
    ];

    // Control: a trailing comment on a statement that stays in place. Upstream
    // and rsvelte agree, so the axis is not "an earlier comment exists".
    assert_eq!(
        compile_cell("\tconst q = 1; // pre\n\texport let v = 1; // c"),
        oracle,
        "a comment trailing a kept statement must not disturb the carry"
    );

    // Residue: an own-line comment before the declaration.
    assert_eq!(
        compile_cell("\t// pre\n\texport let v = 1; // c"),
        vec!["let v = $.fallback($$props['v'], 1); // c".to_string()],
        "own-line comment: upstream keeps the trailing comment inside the call"
    );

    // Residue: the trailing comment of a hoisted `import`.
    assert_eq!(
        compile_cell("\timport { marked } from 'marked'; // ic\n\texport let v = 1; // c"),
        vec!["let v = $.fallback($$props['v'], 1); // c".to_string()],
        "hoisted import comment: same collapse, reached from a different owner"
    );
}
