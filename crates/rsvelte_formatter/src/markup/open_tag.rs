...
pub(super) fn push_open_tag(
    ...
    regular_element: bool,
    // Columns the enclosing block's closing tags consume after this element on
    // the same flat line. An element's own close tag is inside its span and is
    // already measured; a block's `{/if}` is not, so without this the element is
    // judged to fit by exactly the closers' width.
    trailing_close_width: usize,
    options: &FormatOptions,
    ...
    let (one_liner, open_one_line_width) = one_line_open_tag(
        ...
        &rendered_attrs,
        self_closing,
        leading_indent_width + trailing_close_width,
        tw,
    );
