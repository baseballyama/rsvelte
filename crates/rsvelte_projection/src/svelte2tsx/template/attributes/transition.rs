...
pub fn format_transition_directive_v4(name: &str, expr: Option<&str>, tag: &str) -> String {
    expr.map_or_else(|| format!("__sveltets_2_ensureTransition({name}(svelteHTML.mapElementTag('{tag}')));"), |expr_text| {
        format!(
            "__sveltets_2_ensureTransition({name}(svelteHTML.mapElementTag('{tag}'),({expr_text})));"
        )
    })
}
...
pub fn format_animate_directive_v4(name: &str, expr: Option<&str>, tag: &str) -> String {
    expr.map_or_else(|| format!(
        "__sveltets_2_ensureAnimation({name}(svelteHTML.mapElementTag('{tag}'),__sveltets_2_AnimationMove));"
    ), |expr_text| {
        format!(
            "__sveltets_2_ensureAnimation({name}(svelteHTML.mapElementTag('{tag}'),__sveltets_2_AnimationMove,({expr_text})));"
        )
    })
}
...
/// suffix form yet.
pub fn format_transition_directive(transition: &TransitionDirective, source: &str) -> String {
