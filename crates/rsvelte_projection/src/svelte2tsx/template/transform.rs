...
/// `surroundWith` (`node-utils.ts:145-163`). The quotes have to map too, so
/// they are written onto the range's own first and last character rather than
/// beside it; the range then stays one transformation.
pub(crate) fn surround_with(
    ...
    let first_end = start + first.len_utf8() as u32;
    if first_end == end {
        let replacement = format!("{prefix}{first}{suffix}");
        str.overwrite_content_only(start, end, &replacement);
        return;
    }
    let Some(last) = text
        .get(start as usize..end as usize)
    ...
    let last_start = end - last.len_utf8() as u32;
    let head = format!("{prefix}{first}");
    let tail = format!("{last}{suffix}");
    str.overwrite_content_only(start, first_end, &head);
    str.overwrite_content_only(last_start, end, &tail);
}
