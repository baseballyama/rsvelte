...
/// Scan backward from `expr_start` to find the byte offset of the leading `t`
/// in `this=` (for `<svelte:component this={...}>` / `<svelte:element this={...}>`).
///
/// The parser filters `this` out of `el.attributes` and stores only the inner
