...
//!
//! Thin wrappers over the engine-only [`crate::json_api`] functions — the SAME
//! functions the wasm export ([`crate::wasm`]) calls — so the native `.node`
//! addon and the wasm module return byte-identical JSON. Shipped inside the
//! per-platform `@rsvelte/lint-<triple>` packages as `rsvelte_lint.node` and
...
//! on `rsvelte_core/native` (not `rsvelte_core/napi`), and `rsvelte_core` only
//! installs an allocator at its binary entry points — never at the lib level — so
//! nothing pulls a `#[global_allocator]` into this cdylib. The `.node` addon
//! therefore uses the system allocator, which is inherently dlopen-safe: there is
//! no jemalloc/mimalloc initial-exec TLS block to exhaust when Node loads it.
...
pub fn lint(source: String, filename: String) -> String {
    crate::json_api::lint(&source, &filename)
}

/// The full catalog of diagnostic ids [`lint`] can emit (see
/// [`crate::json_api::lint_rules`]).
