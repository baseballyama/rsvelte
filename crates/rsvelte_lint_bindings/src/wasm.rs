...
//! thin `#[wasm_bindgen]` wrappers over the engine-only `json_api` functions.
//!
//! The wrappers are shared verbatim with the NAPI export, so native and wasm return
//! byte-identical JSON. The `rsvelte_core` compiler's own wasm exports (`parse_svelte`,
