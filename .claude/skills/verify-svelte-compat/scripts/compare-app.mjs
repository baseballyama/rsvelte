...
if (!target || !bindingPath || !outputPath) {
  console.error(
    "usage: compare-app.mjs --target <path> --rsvelte-binding <abs.node> --output <json>",
  );
  process.exit(64);
}
...
for (const file of files) {
  ...
  for (const mode of MODES) {
    ...
    try {
      jsResult = svelte.compile(source, opts);
    } catch (e) {
      jsError = String(e?.message || e);
    }
    try {
      rsResult = rsvelte.compile(source, opts);
    } catch (e) {
      rsError = String(e?.message || e);
    }
    ...
    if (jsError) {
      ...
      summary.details.push({
        file: rel,
        mode,
        category: "official-error",
        message: jsError.slice(0, 400),
      });
      continue;
    }
    if (rsError) {
      ...
      summary.details.push({
        file: rel,
        mode,
        category: "rsvelte-error",
        message: rsError.slice(0, 400),
      });
      continue;
    }
