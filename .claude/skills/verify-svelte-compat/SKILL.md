...
1. **新規リポジトリの取り込み** — GitHub URL を受け取り、`compat-targets/<name>/` 配下に git submodule として追加し、互換性を検証する
...
```
compat-targets/                       # このスキルが管理するサブモジュール置き場
...
既存の `vite-plugin-svelte/`、`language-tools/` は **ルート直下のレガシー配置**。このスキルでも Mode B から扱えるが、新規追加は **必ず `compat-targets/<name>/`** に配置する。

`compat-targets/` 自体は git に追加するが、`compat-targets/.cache/` および `compat-targets/*/.compat-meta.json` は `.gitignore` 対象。
...
- Mode B: パスの basename（例: `compat-targets/kit` → `kit`、`vite-plugin-svelte` → `vite-plugin-svelte`）
...
TARGET_PATH="compat-targets/$NAME"
...
- **`alreadySwapped: true`** （ターゲットが既に `@rsvelte/compiler` を import）
  ...
  ```
  これは `compat-targets/.cache/rsvelte-compiler-pkg/` に最小 npm パッケージを作り、ターゲット `package.json` の `pnpm.overrides['@rsvelte/compiler']` を file: 参照に書き換える（**ベースラインのため public 版が要る場合は事前にコピー保存**）。
...
```bash
RSVELTE_LOG="compat-targets/.cache/${NAME}-rsvelte.log"
...
diff "$BASELINE_LOG" "$RSVELTE_LOG" > "compat-targets/.cache/${NAME}-diff.log" || true
```
...
node .claude/skills/verify-svelte-compat/scripts/compare-app.mjs \
  --target "$TARGET_PATH" \
  --rsvelte-binding "$(pwd)/${TARGET_PATH}/.rsvelte/${NODE_NAME}" \
  --output "compat-targets/.cache/${NAME}-app-report.json"
```
...
```bash
# baseline build
( cd "$TARGET_PATH" && pnpm build ) > "compat-targets/.cache/${NAME}-build-baseline.log" 2>&1
# rsvelte build (compiler swap 適用)
( cd "$TARGET_PATH" && NODE_OPTIONS="--import ./.rsvelte/loader.mjs" pnpm build ) \
  > "compat-targets/.cache/${NAME}-build-rsvelte.log" 2>&1
```
...
これは `compat-targets/<name>/.compat-meta.json` に保存し、`.gitignore` 対象にする。
代わりにルートの `compat-targets/index.json` に **要約のみ** を git に含めて履歴にする:
...
# 最後に compat-targets/ 関連の変更をまとめて:
git add compat-targets/index.json .gitmodules compat-targets/<name>
