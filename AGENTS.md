...
| Runtime Runes | 945/979 | 34 skipped — async-blocker / `@const` clusters (Svelte 5.54.1–5.55.9). HtmlTag `is_controlled` + derived-update-server + derived-dep-set-while-rendering + derived-name-shadowed + set-text-stable-coercion + attribute-parts + async-derived-title-update + async-inspect-build + sync-statement grouping + per-const-tag `@const` blocker (Svelte 5.55.3) + `async-eager-derived` (latest-use blocker order + server `$state.eager` derived-skip, Svelte 5.53.x) ported. |
...
**Compatibility report total: 3448/3448 in-scope-run passing — every executed fixture in every in-scope category passes. 38 in-scope fixtures remain skipped (see [docs/skip-remaining-clusters.md](docs/skip-remaining-clusters.md)); the 76 `migrate` fixtures are intentionally out of scope.**
