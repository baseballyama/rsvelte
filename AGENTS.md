...
| Runtime Runes | 990/996 | 6 failing. NEW in 5.56.1: `async-derived-prop-closure-hydration` (#18352) — `$derived(await …)` referenced in a closure prop needs a getter-wrapped component prop on the client plus a server `$.async_derived` unthunk + block-structure pass (async-derived lowering not yet ported). `declaration-tag-multiple-declarators` (#18348) now passes — parser produces a real multi-declarator `VariableDeclaration`, leading-whitespace `{ let … }` is accepted, the client rejoins the split `let`s into one comma declaration, and the server wraps cross-tag derived reads (`doubled()`). Pre-existing (5.56.0): `declaration-tags` (`{const}` block-scope wrap), `async-declaration-tag` / `async-declaration-tag-2` (`metadata.promises_id` lowering), `async-each-const-await-iife` (#18309), `async-style-after-await`. |
...
**Compatibility report total (Svelte 5.56.1): 3509/3515 in-scope-run passing (99.8%). 6 fixtures still failing (all `runtime-runes`). The 5.56.1 bump was entirely DeclarationTag bug-fixes (#18330/#18348/#18350/#18352/#18353); all six were ported, fixing 5 of the 6 fixtures they touched: loose `{let x = a / }` (#18353), unterminated-tag `unexpected_eof` (#18350), `type`-identifier-vs-type-alias disambiguation + interior-comment attachment (#18330), multi-declarator parsing + leading-whitespace tolerance + client comma-rejoin + server cross-tag derived-access + division-after-string in `find_matching_bracket` (#18348/#18353), and the `state_referenced_locally` warning for declaration tags (#18348). The 6 remaining are all in the async-blocker / async-derived / `{const}` block-scope lowering clusters that are not yet fully ported: `async-derived-prop-closure-hydration` (#18352 — async-derived getter-prop on the client + server `$.async_derived` unthunk/block-structure), plus the five pre-existing 5.56.0 clusters `declaration-tags` (`{const}` block-scope wrap), `async-declaration-tag` / `async-declaration-tag-2` (`metadata.promises_id` lowering), `async-each-const-await-iife` (#18309), and `async-style-after-await`. The 76 `migrate` fixtures are intentionally out of scope.**
