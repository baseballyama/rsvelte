...
Source: `pnpm run compatibility-report` (generated 2026-06-02, Svelte commit `3ef761b87b84`, **v5.56.1**). Re-run `pnpm run test-and-update` to refresh. Skip lists live in `crates/rsvelte_core/tests/compatibility_report.rs` and `crates/rsvelte_core/tests/runtime.rs`; `crates/rsvelte_core/tests/audit_skipped.rs` re-checks every skipped fixture after a Svelte bump. See [docs/skip-remaining-clusters.md](docs/skip-remaining-clusters.md) for a per-cluster breakdown of remaining skips with upstream commits, root causes, and a porting plan.

| Suite | Pass/Total | Notes |
|-------|------------|-------|
| Parser Modern | 25/26 | 1 failing (`tag-type-identifier-probe-comment` — Svelte 5.56.1 #18330: `{type instanceof /* probe */ Object}` now correctly parses as an *expression* tag rather than a malformed type declaration, but the interior `/* probe */` comment is recorded only in `Root.comments` (with a `character` loc field) instead of also being attached as `leadingComments` on the `Object` node — rsvelte's expression-comment converter only attaches comments leading/trailing the *whole* expression, not interior sub-nodes). `loose-declaration-tag` now passes (5.56.1 #18353 loose `{let x = a / }` → empty-name declarator). |
| Parser Legacy | 82/82 | |
...
| Validator | 332/332 | All executed fixtures pass — `declaration-tag-state-referenced-locally` (Svelte 5.56.1 #18348) ported: the DeclarationTag analyze visitor structurally replicates the `state_referenced_locally` check (warn on a reactive binding read at the top level of an initializer, skipping closures and `$state`/`$derived` call arguments). `declaration-tag-invalid-type-2` (5.56.1 #18330) passes — `{type instanceof Foo}` / `{type in foo}` etc. are no longer misclassified as type declarations. |
...
| Runtime Runes | 989/996 | 7 failing. NEW in 5.56.1: `declaration-tag-multiple-declarators` (#18348) — parser now produces a real multi-declarator `VariableDeclaration` (so scope/analyze see both `count` and `doubled`) and leading-whitespace `{ let … }` is accepted, and the runtime behavior is correct, but the client transform still lowers the two declarators into two separate `let` statements rather than one comma-separated statement (instance-script lowering splits comma declarators), so the generated JS text differs; `async-derived-prop-closure-hydration` (#18352) — `$derived(await …)` referenced in a closure prop needs a getter-wrapped prop on the client plus a server `$.save`-free `references` blocker pass (async-derived lowering not yet ported). Pre-existing (5.56.0): `declaration-tags` (`{const}` block-scope wrap), `async-declaration-tag` / `async-declaration-tag-2` (`metadata.promises_id` lowering), `async-each-const-await-iife` (#18309), `async-style-after-await`. |
...
**Compatibility report total (Svelte 5.56.1): 3507/3515 in-scope-run passing (99.8%). 8 fixtures still failing (`runtime-runes`: 7 + `parser-modern`: 1). The 5.56.1 bump was entirely DeclarationTag bug-fixes (#18330/#18348/#18350/#18352/#18353); ported here: loose `{let x = a / }`, unterminated-tag `unexpected_eof`, `type`-identifier-vs-type-alias disambiguation, multi-declarator parsing, leading-whitespace tolerance, division-after-string in `find_matching_bracket`, and the `state_referenced_locally` warning for declaration tags. The 8 remaining are representation/lowering gaps in deep machinery rather than parse/behavior bugs: interior-comment `leadingComments` attachment (`tag-type-identifier-probe-comment` — the `{type instanceof X}` headline parse is fixed; only the `/* probe */` comment's attachment to the `Object` node differs), multi-declarator client codegen formatting (`declaration-tag-multiple-declarators` — parses/scopes/runs correctly, but the client lowers the two declarators into two `let` statements instead of one comma-separated statement), the async-derived getter-prop lowering (`async-derived-prop-closure-hydration`), plus the five pre-existing 5.56.0 `{const}` block-scope / async-blocker clusters (`declaration-tags`, `async-declaration-tag`, `async-declaration-tag-2`, `async-each-const-await-iife`, `async-style-after-await`). The 76 `migrate` fixtures are intentionally out of scope.**
