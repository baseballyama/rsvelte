...
| Runtime Runes | 992/996 | 4 failing, all pre-existing 5.56.0 clusters: `declaration-tags` (nested `{const}`/`{let}` inside an element needs a block-scope `{ … }` wrap + constant-folding of the shadowed binding), `async-declaration-tag` / `async-declaration-tag-2` (the `metadata.promises_id` lowering for `{let x = $state(await …)}` is not yet ported), and `async-style-after-await` (a shorthand `style:x` directive after a top-level `await` over-emits `$$promises` blockers + script-comment preservation in the hoisted `var`). The 5.56.1 `declaration-tag-multiple-declarators` (#18348) and `async-derived-prop-closure-hydration` (#18352) now pass, and the pre-existing `async-each-const-await-iife` (#18309) was unblocked by collecting blockers from inside memoized-thunk closures. |
...
**Compatibility report total (Svelte 5.56.1): 3511/3515 in-scope-run passing (99.9%). 4 fixtures still failing (all `runtime-runes`). The 5.56.1 bump was entirely DeclarationTag bug-fixes (#18330/#18348/#18350/#18352/#18353); all six were ported, fixing every fixture they touched: loose `{let x = a / }` (#18353), unterminated-tag `unexpected_eof` (#18350), `type`-identifier-vs-type-alias disambiguation + interior-comment attachment (#18330), multi-declarator parsing + leading-whitespace + client comma-rejoin + server cross-tag derived-access + division-after-string (#18348/#18353), the `state_referenced_locally` warning (#18348), and the async-derived component-prop getter + server `$.async_derived` unthunk (#18352). Two pre-existing 5.56.0 async fixtures were also unblocked along the way (`async-each-const-await-iife` #18309 — collect blockers from inside memoized-thunk closures). The 4 remaining are all pre-existing 5.56.0 clusters needing deeper, not-yet-ported infrastructure: `declaration-tags` (element-nested `{const}`/`{let}` block-scope wrap + constant-folding), `async-declaration-tag` / `async-declaration-tag-2` (`metadata.promises_id` lowering for `{let x = $state(await …)}`), and `async-style-after-await` (shorthand-`style:` blocker over-emission after a top-level await + script-comment preservation). The 76 `migrate` fixtures are intentionally out of scope.**
