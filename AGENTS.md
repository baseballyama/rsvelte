...
| Runtime Runes | 961/979 | 18 skipped — async-blocker / `@const` clusters (Svelte 5.54.1–5.55.9). HtmlTag `is_controlled` + derived-update-server + derived-dep-set-while-rendering + derived-name-shadowed + set-text-stable-coercion + attribute-parts + async-derived-title-update + async-inspect-build + sync-statement grouping + per-const-tag `@const` blocker (Svelte 5.55.3) + `async-eager-derived` (Svelte 5.53.x) + `{#await await ...}` async-batching (Svelte 5.55.9) + SSR `$.save` parent-walk predicate ported. |
...
**Compatibility report total: 3464/3464 in-scope-run passing — every executed fixture in every in-scope category passes. 22 in-scope fixtures remain skipped (see [docs/skip-remaining-clusters.md](docs/skip-remaining-clusters.md)); the 76 `migrate` fixtures are intentionally out of scope.**
