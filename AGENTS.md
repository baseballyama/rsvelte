...
| Validator | 331/332 | 1 failing (`declaration-tag-state-referenced-locally` — Svelte 5.56.1 #18348 expects a `state_referenced_locally` warning for a non-closure `{let e = $state(0), f = e}` read; rsvelte's declaration-tag analyze visitor uses the specialized `walk_js_expression` walker, which does not run the main Identifier visitor where that warning is emitted, so no warning fires). `declaration-tag-invalid-type-2` (5.56.1 #18330) passes — `{type instanceof Foo}` / `{type in foo}` etc. are no longer misclassified as type declarations. |
...
**Compatibility report total (Svelte 5.56.1): 3506/3515 in-scope-run passing (99.7%). 9 fixtures still failing (`runtime-runes`: 7 + `parser-modern`: 1 + `validator`: 1). The 5.56.1 bump was entirely DeclarationTag bug-fixes (#18330/#18348/#18350/#18352/#18353); ported here: loose `{let x = a / }`, unterminated-tag `unexpected_eof`, `type`-identifier-vs-type-alias disambiguation, multi-declarator parsing, leading-whitespace tolerance, and division-after-string in `find_matching_bracket`. The remaining failures are representation/lowering gaps in deep machinery rather than parse/behavior bugs: interior-comment `leadingComments` attachment (`tag-type-identifier-probe-comment`), the `state_referenced_locally` warning path for declaration tags (`declaration-tag-state-referenced-locally`), multi-declarator client codegen formatting (`declaration-tag-multiple-declarators`), the async-derived getter-prop lowering (`async-derived-prop-closure-hydration`), plus the five pre-existing 5.56.0 `{const}` block-scope / async-blocker clusters (`declaration-tags`, `async-declaration-tag`, `async-declaration-tag-2`, `async-each-const-await-iife`, `async-style-after-await`). The 76 `migrate` fixtures are intentionally out of scope.**
