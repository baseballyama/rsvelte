...
for (const p of [
  ...
  try { rsvelte = require(p); break; } catch { /* next */ }
}
if (!rsvelte) { console.error('No rsvelte binding found'); process.exit(1); }
...
let totalFiles = 0, totalBothCompiled = 0, totalSemEqual = 0;
let totalSemDiff = 0, totalRsErr = 0, totalJsErr = 0, totalBothErr = 0;
...
for (const project of PROJECTS) {
  ...
  let semEqual = 0, semDiff = 0, rsErr = 0, jsErr = 0, bothErr = 0;
  ...
  for (const file of files) {
    ...
    try { jsCode = svelte.compile(source, opts).js.code; } catch (e) { jsError = e.message; }
    try { rsCode = rsvelte.compile(source, sanitizeOptions(opts)).js.code; } catch (e) { rsError = e.message; }

    if (jsError && rsError) { bothErr++; continue; }
    if (jsError) { jsErr++; continue; }
    if (rsError) {
      rsErr++;
    ...
    if (jsCan === rsCan) {
      ...
      if (semDiffDetails.length < 15) {
        ...
        semDiffDetails.push({
          project: project.name, file: relPath, line: di + 1,
...
console.log(`Both compiled: ${totalBothCompiled}/${totalFiles} (${(totalBothCompiled/totalFiles*100).toFixed(1)}%)`);
