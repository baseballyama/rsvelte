...
outer: for (const proj of projects) {
  ...
  for (const file of files) {
    ...
    let result;
    try {
      result = execFileSync(CANON_BIN, ['/tmp/_js.js', '/tmp/_rs.js'], { encoding: 'utf-8', env: cleanEnv, timeout: 10000 }).trim();
    } catch { continue; }
    if (result === 'MATCH') continue;

    // Dump canonicalized versions
    let canonJs, canonRs;
    try { canonJs = execFileSync(CANON_BIN, ['/tmp/_js.js', '/tmp/_rs.js', '--dump1'], { encoding: 'utf-8', env: cleanEnv }); } catch {}
    try { canonRs = execFileSync(CANON_BIN, ['/tmp/_js.js', '/tmp/_rs.js', '--dump2'], { encoding: 'utf-8', env: cleanEnv }); } catch {}
