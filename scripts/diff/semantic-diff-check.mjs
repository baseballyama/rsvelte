...
for (const proj of projects) {
  ...
  for (const file of files) {
    ...
    if (je) {
      jsErr++;
      jsErrors.push(`${proj.name}/${rel}: ${je.substring(0, 150)}`);
      continue;
    }
    if (re) {
      rsErr++;
  ...
  console.log(
    `${proj.name}: ${projEqual}/${compiled} semantic match (${((projEqual / compiled) * 100).toFixed(1)}%), ${projRsErr} errors`,
  );
}
...
console.log(
  `Semantically equal: ${semEqual}/${compiled} (${((semEqual / compiled) * 100).toFixed(1)}%)`,
);
