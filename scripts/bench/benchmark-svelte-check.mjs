 ...
 *
 * Usage:
 *   node scripts/bench/benchmark-svelte-check.mjs [--files=N] [--runs=K] [--js]
 *
...
try {
  ...
  timeMs(
    ...
    () => {
      spawnSync(
        RSVELTE_BIN,
        ["--workspace", fixture, "--emit-overlay", "--incremental", "--output", "machine"],
        { stdio: "ignore" },
      );
    },
  ...
  if (haveTsgo) {
    ...
    console.log("  (skipped --tsgo run — no tsgo / tsc on PATH; set TSGO_BIN to enable)");
  }
  ...
  if (COMPARE_JS) {
    ...
    if (localExists) {
      ...
      timeMs("node submodules/.../svelte-check", () => {
        spawnSync("node", [JS_SVELTE_CHECK_BIN, "--workspace", fixture, "--output", "machine"], {
          stdio: "ignore",
        });
      });
    } else {
      ...
      timeMs("npx svelte-check", () => {
        spawnSync("npx", ["--yes", "svelte-check", "--workspace", fixture, "--output", "machine"], {
          stdio: "ignore",
        });
      });
    }
