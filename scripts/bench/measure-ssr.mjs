...
for (const file of allFiles) {
  ...
  if (count % 50 === 0 || count === allFiles.length) {
    ...
    try {
      fs.writeFileSync(
        "/tmp/ssr_measure_result.txt",
        `Canon match: ${canonMatch}/${ct} (${ct ? ((canonMatch / ct) * 100).toFixed(1) : 0}%)\nProcessed: ${count}/${allFiles.length}\n`,
      );
    } catch {}
  }
}
...
try {
  ...
  fs.writeFileSync(
    "/tmp/ssr_measure_result.txt",
    `Canon match: ${canonMatch}/${canonTotal2} (${canonTotal2 ? ((canonMatch / canonTotal2) * 100).toFixed(1) : 0}%)\n`,
  );
} catch {}
