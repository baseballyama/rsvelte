...
for (const file of allFiles) {
  ...
  try {
    ...
    results.client.jsErr++;
    continue;
  }
  try {
    rc = rsvelte.compile(src, opts).js.code;
  } catch {
    results.client.rsErr++;
    continue;
  }
  if (jc === rc) {
    results.client.rawMatch++;
    results.client.canonMatch++;
  } else {
    ...
    if (clientRawDiffSamples.length < 5) {
      ...
      clientRawDiffSamples.push({
        rel,
        js: JSON.stringify(jc.substring(pos, pos + 80)),
...
// Print client results first (SSR may crash)
{
  ...
  console.log(`\n=== CLIENT ===`);
  console.log(
    `Raw match:   ${r.rawMatch}/${compiled} (${compiled ? ((r.rawMatch / compiled) * 100).toFixed(1) : 0}%)`,
  );
  console.log(
    `Canon match: ${r.canonMatch}/${canonTotal} (${canonTotal ? ((r.canonMatch / canonTotal) * 100).toFixed(1) : 0}%)`,
  );
...
for (const file of allFiles) {
  ...
  try {
    ...
    results.ssr.rsErr++;
    if (ssrRsErrors.length < 10)
      ssrRsErrors.push(`${rel}: ${String(e?.message || e).substring(0, 150)}`);
    continue;
  }
  if (!rc) {
    results.ssr.rsErr++;
  ...
  if (jc === rc) {
    results.ssr.rawMatch++;
    results.ssr.canonMatch++;
  } else {
    results.ssr.rawDiff++;
    if (canonCompare(jc, rc)) results.ssr.canonMatch++;
      ...
      if (ssrCanonDiffs.length < 15) {
        ...
        ssrCanonDiffs.push({
          rel,
          js: jc.substring(pos, pos + 100),
          rs: rc.substring(pos, pos + 100),
        });
      }
    }
  }
