 ...
 *   node scripts/dev/update-docs.mjs --report path/to/report.json
 */
...
function updateSvelteTargetMarker(content, version, commitHash) {
  ...
  const markerRegex = /<!-- svelte-target-version -->[\s\S]*?<!-- \/svelte-target-version -->/;
...
function updateReadme(_report) {
  ...
  if (updated !== original) {
    ...
    console.log(
      `README.md Svelte target marker already up to date (v${version} @ ${commit.slice(0, 12)})`,
    );
  }
}
...
function checkReadmeInSync(_report) {
  ...
  if (expected !== original) {
    console.error("README.md Svelte target-version marker is out of sync with the submodule.");
...
function main() {
  ...
  console.log(
    `Overall: ${report.summary.total_passed}/${report.summary.total_tests - report.summary.total_skipped} (${report.summary.overall_percentage.toFixed(1)}%)\n`,
  );
