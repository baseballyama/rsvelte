...
function benchmark(name, source, iterations = 100) {
    ...
    console.log(`--- ${name} ---`);
    console.log(`  Client: mean=${clientMean.toFixed(3)}ms, median=${clientMedian.toFixed(3)}ms, min=${clientMin.toFixed(3)}ms`);
    console.log(`  Server: mean=${serverMean.toFixed(3)}ms, median=${serverMedian.toFixed(3)}ms, min=${serverMin.toFixed(3)}ms`);
    console.log(`  Throughput (client): ${((source.length / 1024 / 1024) / (clientMean / 1000)).toFixed(2)} MiB/s`);
    console.log(`  Throughput (server): ${((source.length / 1024 / 1024) / (serverMean / 1000)).toFixed(2)} MiB/s`);
