...
let jsParseFails = 0, rsParseFails = 0, bothParse = 0;
const rsFailFiles = [];
...
for (const f of all) {
  ...
  let jsParseOk = false, rsParseOk = false;
  try {
    // Use a simple parse check via Function constructor won't catch syntax errors for modules
    // Instead, write a parse script
    execFileSync('node', ['--input-type=module', '-e', `import('data:text/javascript;base64,${Buffer.from(jc).toString('base64')}').catch(e => { if (e.message.includes('Unexpected') || e.message.includes('parsing')) process.exit(1); });`], { env: cleanEnv, timeout: 5000, stdio: 'pipe' });
    jsParseOk = true;
  } catch {}

  try {
    execFileSync('node', ['--check', '/tmp/_r.js'], { env: cleanEnv, timeout: 5000, stdio: 'pipe' });
    rsParseOk = true;
  } catch {}
