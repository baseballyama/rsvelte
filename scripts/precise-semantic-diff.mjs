...
for (const proj of projects) {
  ...
  for (const file of files) {
    ...
    if (re) { rsErr++; rsErrors.push(`${proj.name}/${rel}: ${re.substring(0, 200)}`); continue; }
    ...
    try {
      ...
      diff++; pd++;
      if (diffs.length < 30) diffs.push({ file: `${proj.name}/${rel}`, f1: 'CANON_ERROR', f2: String(e.message).substring(0, 100) });
    }
  }
