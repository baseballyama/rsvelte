...
if (!input) {
  console.error(
    "usage: node scripts/compat-corpus/one.mjs <corpus-id-or-file> [--target client|server] [--raw]",
  );
  process.exit(2);
}
...
function fmt(code, name) {
  ...
  try {
    execFileSync(
      "npx",
      ["oxfmt", "-c", path.join(CORPUS, ".oxfmtrc.json"), "--ignore-path", "/dev/null", tmp],
      { stdio: "pipe" },
    );
  } catch {
...
for (const target of targetArg ? [targetArg] : ["client", "server"]) {
  ...
  if (e.error || a.error) {
    console.log(
      "expected:",
      e.error ? `ERROR ${e.error.code}: ${e.error.message.split("\n")[0]}` : "compiles",
    );
    console.log(
      "actual:  ",
      a.error ? `ERROR ${a.error.code}: ${a.error.message.split("\n")[0]}` : "compiles",
    );
    continue;
  }
  const ef = fmt(e.js, "e");
