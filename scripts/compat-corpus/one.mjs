...
for (const target of selected) {
	...
	if (e.error || a.error) {
		// The whole span is printed because verify.mjs ratchets `start` and `end`
		// separately: an entry can fail either while both codes and messages read
		// identical.
		const show = (r) =>
			r.error
				? `ERROR ${r.error.code} @${r.error.line ?? '?'}:${r.error.column ?? '?'}-${r.error.endLine ?? '?'}:${r.error.endColumn ?? '?'}: ${r.error.message.split('\n')[0]}`
				: 'compiles';
		console.log('expected:', show(e));
		console.log('actual:  ', show(a));
		if (e.error && a.error && (e.error.frame ?? null) !== (a.error.frame ?? null)) {
			console.log('--- expected frame ---\n' + (e.error.frame ?? '(none)'));
			console.log('--- actual frame ---\n' + (a.error.frame ?? '(none)'));
		}
		continue;
	}
	const ef = fmt(e.js, 'e');
