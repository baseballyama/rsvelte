...
async function main() {
  ...
  const haveMeta = fs.existsSync(META_PATH) ? JSON.parse(fs.readFileSync(META_PATH, "utf8")) : null;
  const oracleFresh =
    haveMeta &&
  ...
  if (!ONLY_ACTUAL && (FORCE || !oracleFresh)) {
    ...
    await pool(components, async ({ id }) => {
      ...
      const res = await exec(
        OXFMT_BIN,
        ["-c", OXFMT_CONFIG, "--stdin-filepath", stdinName(id)],
        source,
      );
    ...
    console.log(
      `[fmt] oracle: ${included.length} included, ${skips.length} skipped (not valid/formattable svelte)`,
    );
  } else if (!ONLY_ACTUAL) {
    console.log(
      `[fmt] oracle: up to date (${included.length} included) — use --force to regenerate`,
    );
  }
  ...
  if (!ONLY_ORACLE) {
    if (!fs.existsSync(RSVELTE_FMT_BIN)) {
      fail(
        `rsvelte-fmt not found at ${RSVELTE_FMT_BIN} — run \`cargo build --release -p rsvelte_fmt\` or set RSVELTE_FMT_BIN`,
      );
    }
    ...
    if (ONLY_FILE) {
      ...
      console.log(
        `[fmt] actual: --only ${path.relative(ROOT, ONLY_FILE)} → ${targets.length} of ${included.length}`,
      );
    } else {
