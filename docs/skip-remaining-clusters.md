...
| 5.55.4 `@const` context — `async-effect-pending-eager` (1 fixture remaining) | `0ed8c282f` "fix: reset context after waiting on blockers of @const expressions" | ✅ Const-async wrapping in component wrapper path landed: `apply_const_async_wrapping` now runs in the `$$renderer.component(...)` wrapper branch of `3_transform/server/build.rs::build_program` (in addition to the existing call in `build()`), so `{@const foo = bar}` references — where `bar` is a top-level `$$promises[N]` blocker — correctly wrap dependent text expressions in `$$renderer.async([promises[M]], ($$renderer) => $$renderer.push(() => $.escape(foo)))`. The wait + assignment thunks were already split (since the 5.55.3 port); the missing piece was that the wrapper rebuilds `inner_body` from `self.output_parts` and only `apply_async_wrapping` (top-level `$$promises`) was being applied, silently dropping const-tag-level blockers. Unblocked `runtime-runes/async-context-after-await-const`. `async-effect-pending-eager` (added in upstream `273f1a85a`, not `0ed8c282f`) still needs `$effect.pending()` rewriting in `{#if}` test expressions (currently only handled in `{...}` expression tags via `transform_rune_in_template_expr`) and `<p>pending: {...}</p>` trailing-whitespace normalisation — both orthogonal to the const-tag context fix. |
...
| `async-boundary-nav-race`, `async-if-else` (2 fixtures, **mislabelled 5.53.4**) | not `3a289797b` — see notes | Investigation found these don't depend on the default-parameter porosity fix. rsvelte already handles that case correctly (the bundled `function-parameter-shadowing` fixture passes byte-for-byte). The two fixtures actually fail on: (a) server `$.save(...)` predicate — `has_save: !self.in_block_body` in `server/visitors/expression_tag.rs:68` is too coarse; upstream `AwaitExpression.js` walks parents and applies `save` when a metadata-bearing non-Fragment/ExpressionTag parent is hit, so awaits inside `$$renderer.child_block(async ...)` *do* get wrapped; (b) `$$renderer.child_block(async ($$renderer) => { ... })` body is under-indented by one tab; (c) for `async-boundary-nav-race` only: snippet functions are hoisted to module scope instead of staying inside `Main(...)`. Same root cause as the 5.55.1 / 5.55.6 `$.save`-predicate work — fix those clusters and these 2 follow. |
