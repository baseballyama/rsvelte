...
| ~~5.54.1 cluster~~ — `async-style-after-await` (1 fixture remaining; client-side) | `6b33dd2a1` "fix: group sync statements" | ✅ Sync-statement grouping ported (`SyncBlock(Vec<AsyncStmt>)` in `transform_async_body_inner`; mirrored in `compute_blocker_map`). Unblocked `async-if-hydration`, `async-derived-with-effect-and-boundary`, `async-binding-after-await`, `async-transform-empty-statements`. ✅ SSR `$.save` parent-walk predicate ported (`in_block_body` now defaults to `true`, toggled `false` only by element-like visitors — `RegularElement` / `TitleElement` / `select` / `textarea` / `<option>`). Unblocked `async-derived-indirect`, `async-later-sync-overlaps`. `async-style-after-await` still fails on the client side (orthogonal cluster). |
| ~~`async-overlap-multiple-1..7`~~ (5.55.1 `5e8662fb2`, **landed**) | "chore: lots of async tests" | ✅ SSR `$.save` parent-walk predicate ported (see above row). Unblocked `async-overlap-multiple-1..4`. ✅ Transitive `touch`-through-`binding.assignments` ported in `3_transform/shared/async_body.rs::compute_blocker_map` + `update_blocker_map_for_stmt` — when a later async statement references a binding, the walk also follows that binding's initializer expression (via a new `collect_var_init_map`) so identifiers reachable through the assignment chain get the same (higher) blocker index. Mirrors upstream's `calculate_blockers` → `touch(call)` → `binding.assignments` recursion in `2-analyze/index.js`. Unblocked `async-overlap-multiple-5..7` (the `let b = $derived(await delay(a*2))` → `let d = $derived(await delay(b + c))` chain now collapses `a`/`b`/`c`/`d` to the chained derived's index, matching upstream's `[$$promises[2]]` blockers array). |
...
| ~~5.55.9 cluster~~ — `async-await-block-2`, `async-duplicate-dependencies` (0 fixtures remaining) | `000c594e0` "fix: `{#await await ...}` and async dependencies fixes" | ✅ `{#await await ...}` async-batching codegen ported (unblocked `runtime-runes/async-await`). ✅ Follow-up ports landed: (a) `$derived(await ...)` inside a *nested* instance-script function now lowers to `(await $.save($.async_derived(...)))()` instead of bare `await $.async_derived(...)`. Added a `function_depth` counter to the client `ast_state_transform::StateVarCollector` (incremented in `visit_function` / `visit_arrow_function_expression`); `try_rewrite_derived_call_declarator`'s async branch checks `function_depth >= 1` to choose the `save`-wrapped emit, mirroring upstream `VariableDeclaration.js`'s `is_instance && function_depth > 1` check. (b) Server `{:then result}` / `{:catch err}` await-block parameter bindings now shadow outer derived names: `3_transform/server/visitors/await_block.rs::generate_await_block` removes the param's leaf names (via `each_block::extract_pattern_names`, which was made `pub(crate)`) from the inherited `derived_names` / `derived_var_names` for the body generator, so `(result) => $$renderer.push(\`${$.escape(result)}\`)` is emitted instead of `$.escape(result())`. Mirrors the same handling for snippet parameters (`snippet_block.rs`) and each-block context patterns. Unblocked `runtime-runes/async-await-block-2`, `runtime-runes/async-duplicate-dependencies`. |
| `async-boundary-nav-race` (1 fixture remaining; **mislabelled 5.53.4**) | not `3a289797b` — see notes | rsvelte already handles default-parameter scope leak correctly. ✅ SSR `$.save` parent-walk predicate ported (unblocked `async-if-else`). `async-boundary-nav-race` still fails on orthogonal axes: snippet functions are hoisted to module scope instead of staying inside `Main(...)`, plus `$$renderer.child_block(async ($$renderer) => { ... })` body is under-indented by one tab. |
