...
### ★★ 結論: 「エピックの次の標的は X」はコーパス名を書かない限り偽 ★★
...
**★ 順位逆転は負荷で作れない唯一の証拠クラス ★**
本日このマシンは符号反転を 2 回起こしている（svelte-ux の ON/OFF、carbon の
old/new）。シェアは動きうるが、**同一 run 内の 2 行の順位**は動かない。
`member_mutations` が open-webui で `prop_source_reads` の 9.5 倍、
smelte では**逆に安い** — これは計測ノイズでは作れない。
...
> - バケット内で平坦・バケット間で上昇 → **積モデル**
> - バケット内でも上昇 → **文サイズ単独**が担っている
...
インデックスを `stage × bucket` に広げるだけで済む。
...
ただし**規模は小さい — compile 比で 0.53〜3.49%**。単価の異常さと
仕事の大きさは別の問い（`feedback_numbers_that_size_future_work`）。
