...
**`$:` 密度の関数でしかない**。そして上の 4 つの legacy コーパスは
`$:`/stmt が **10.7〜12.8% と実質同一**＝ **軸上の 1 点**である。
「lib 45〜51% / app 69%」という読みは成立しない — app である carbon/src が
47.4% で lib のレンジのど真ん中に入り、runes app の SMUI が 90.5% で
runes lib の shadcn と並ぶ。**効いているのは lib/app ではなく `$:` の有無と密度。**

**★ したがって元の 69%（carbon 55 / huly 62 / open-webui 85 の集計）は棄却できない。★**
未測定の open-webui 85% は `$:` 密度 2〜3% 相当の**中間点**を意味し、
こちらの 2 クラスタ（12% と 0%）はその点を挟むだけで再現していない。
carbon/src だけは 55% → 実測 47.4% とずれたが、**原因は特定できない**:
(1) warmup 汚染（元は汚染バイナリ）、(2) 元の計算誤り、
(3) **リビジョン差** — 手元の carbon は当時の pin ではなく本日 HEAD の shallow clone。
(4) スコープ差は排除済み（`src/` 291 files で測定。root 1324 files は
tests/docs を含み legacy 密度が 3.2 倍違うので比較不能）。

**再発防止**: 母集団 A で測った数値が母集団 B で再現しないことは、
**その数値の棄却ではない**。「45〜51%」も「69%」も、それぞれの母集団についてのみ真。

### 上位行（`pa_rest` 比）

| stage | smelte | sveltestrap | svelte-ux | carbon/src | µs/call |
|---|---|---|---|---|---|
| **`export_let`** | **29.0%** | **15.2%** | **19.1%** | 11.2% | 2.6–4.0 |
| `state_assigns` | 8.7% | 9.5% | **18.5%** | **34.6%** | 1.0–5.6 |
| `prop_assignments` | 13.5% | 10.9% | 9.9% | 11.1% | 1.3–1.8 |
| `prop_source_reads` | 11.6% | 10.8% | 8.2% | 9.3% | 1.1–1.5 |
| `state_reads` | 6.2% | 8.4% | 8.6% | 7.0% | 0.7–1.2 |
| `destructure_assignments` | 2.9% | 12.2% | 6.4% | 5.8% | 0.3–1.5 |

### ★ 母集団論争に依存しない唯一の数値: `export_let` の単価 ★

シェア表は母集団が決まるまで動く。**単価は動かない。**

| corpus | ms | calls | bytes | **ns/byte** | 同一 run の `prop_assignments` | 倍率 |
|---|---|---|---|---|---|---|
| smelte | 0.954 | 369 | 12411 | **76.9** | 21.4 | 3.6x |
| sveltestrap | 1.109 | 369 | 10774 | **102.9** | 7.2 | 14.2x |
| svelte-ux | 1.878 | 647 | 22129 | **84.9** | 12.1 | 7.0x |
| carbon/src | 6.629 | 1665 | 54645 | **121.3** | 15.2 | 8.0x |

**30〜34 バイトの 1 文に 2.6–4.0µs、77–121 ns/byte。**
比較は**同一 run の兄弟ステージ**と取ること（分母の定義が同じで負荷が相殺される）
— `script_text` バケット全体の ns/byte と比べたくなるが、あちらの分母は
**ファイル全バイト**でこちらは**文のバイト**なので比較できない。
兄弟比では **3.6〜14 倍**。原因は `transform_export_let` →
`state_pipeline_ast` の AST パスが 1 文ごとに走ること。
**`pa_rest` で最初に見るべき行はこれ。**
