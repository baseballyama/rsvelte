...
**結論: このプロファイラで使えるのはカウンタと、n≥6 の min/max レンジだけ。**
単発 run のシェアを表に載せてはいけない（本文で断っても表は測定値に見える）。
`smelte` 規模（≲100 files）は例外的に単発でも ±1.5 倍以内。
...
**★ 追加の前提条件（6 連続 run の結果を受けて）★**: この判別は
**carbon と open-webui でしか成立しない**が、その 2 つは**まさに
シェアが 17〜30 倍ばらつくコーパス**である。したがって
**バケット別の値も n≥6 の min/max で報告し、レンジが重なったら
`unmeasured` とすること**。単発 run のバケット表は判別力を持たない。
この前提が満たせないなら、判別実験そのものが成立しない。

### `export_let` の単価（★ 主張を 2 つに分けて片方だけ残す ★）

6 連続 run × 3 コーパス = 18 run で測り直した結果、
**残るのは絶対単価、落ちるのは兄弟比**:

| corpus | `export_let` ns/byte (n=6 レンジ) | 兄弟 `prop_assignments` 比 | 比の spread |
|---|---|---|---|
| smelte | 77.8–127.9 | 3.03–3.60x | **1.19x** |
| carbon/src | 124.8–200.3（+外れ値 1 run 1727.9） | 0.74–118.79x | **160x** |
| open-webui | 125.6–167.6 | 2.46–9.25x | 3.76x |

**★ 生きている主張 ★**: `export_let` の単価は **~78–200 ns/byte**
（30〜34 バイトの 1 文に 2.6–4.0µs）。**`export_let` 自身は安定していて、
ノイズを持ち込んでいるのは兄弟の方**（carbon の `prop_assignments` は
14.5–174.4 ns/byte で 12 倍動く）。
順序としては **18 run 中 17 run で `export_let` > `prop_assignments`**。

**★ 撤回した主張 ★**: 「兄弟の **3.6〜14 倍**」。これは各コーパス 1 run の
値をコーパス間レンジとして読んだもので、**実体は run 間ノイズ**。
carbon では 1 run が 0.74 倍（＝ `export_let` の方が**安い**）になる。
**超過の倍率は確立していない。**

規模は小さい — compile 比で 0.53〜3.49%（これも単発 run 由来なので要再測）。
**単価の異常さと仕事の大きさは別の問い**
（`feedback_numbers_that_size_future_work`）。
原因は `transform_export_let` → `state_pipeline_ast` の AST パスが 1 文ごとに走ること。
