...
| ≥ 2× faster than JS svelte-check on a 1000-file project                            | ✅     | `scripts/bench/benchmark-svelte-check.mjs --js --files=1000`, both pipelines pointed at the same `tsc` backend (the JS reference's own `submodules/language-tools/packages/svelte-check` build). Sample numbers on the dev machine: rsvelte `--tsgo` end-to-end **1107ms** vs JS svelte-check **3628ms** → **~3.3× speedup**. Warm `--incremental --emit-overlay` is **~70×** faster than JS svelte-check (52ms vs 3628ms) — the cache amortises the overlay materialisation. Cold parse-only (no TS) is ~150×. Re-run with `TSGO_BIN=<…>` set to use tsgo's native TS where available. |
...
5. ~~**Performance benchmark**~~ — ✅ landed
   (`scripts/bench/benchmark-svelte-check.mjs`). The acceptance number
...
| Smoke / fixture tests for the NAPI surface                            | ✅     | `pnpm run test:vps` runs both `scripts/dev/test-vps-shim.mjs` (raw NAPI exports — 9 assertions) and `scripts/dev/test-vps-vite-fixture.mjs` (Vite-shaped end-to-end: preprocess → client/server compile → HMR diff on a real Counter.svelte — 6 assertions). |
...
2. ~~**E2E smoke test**~~ — ✅ landed via the Rust-repo-side scripts
   (`scripts/dev/test-vps-shim.mjs` + `scripts/dev/test-vps-vite-fixture.mjs`).
