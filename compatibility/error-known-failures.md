...
`error-position-known-failures.<target>.json` hold 68 entries, all four of
`error-end-known-failures.<target>.json` hold 82 entries, and all four of
...
The block-header pattern validation branch retired 10 position and 16 end
entries per target. Those shrinks are the `{#each}` / `{#await}` headers whose
errors are now raised at the same token as upstream.
...
By shape (client target, an earlier 92-entry snapshot), classified from the run's own
...
Partition of an earlier `error-end-known-failures.<target>.json` snapshot by
shape: `26 + 47 + 25` (client target, classified from the run's own
`error.json` records):
