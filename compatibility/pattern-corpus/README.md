...
| `2620-keyword-before-regex.svelte` | [#2620](https://github.com/baseballyama/rsvelte/issues/2620) | `typeof /[//]/` — whether a `/` opens a regex literal was decided from the byte before it, and the `f` of `typeof` reads as "an operand ended here". The literal was then scanned as a division, which left its body exposed as code: the `//` inside the character class read as a line comment and the prop behind it went unrewritten |
