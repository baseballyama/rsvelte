...
| `2608-destructured-param-binding.svelte` | [#2608](https://github.com/baseballyama/rsvelte/issues/2608) | A prop name occupying a **binding slot of a destructuring parameter** inside a legacy `$:` statement. The client prop-read rewriter asked only "is this a shorthand object-literal property?", so `({ id }) =>` became `({ id: id() }) =>` and `([id, n]) =>` became `([id(), n]) =>` — binding patterns no parser accepts. The `n = id`, `[id]:` and `(o = { id })` lines pin the other side: a name in a default value, a computed key or an object literal defaulting a parameter is still a read and keeps its `id()` |
