...
`musicat/…/info/MetadataSection.svelte` and `musicat/…/library/SongHighlighter.svelte`
left **22 — intra-line-ws** in the oxfmt 0.67.0 bump, and the mover is the **oracle**, not
rsvelte. Both were a doubled space before a trailing `//` comment on a closing brace; running
the oracle over `MetadataSection.svelte` under both versions with the corpus config, the whole
file differs on exactly one line — `}  // Invalid characters` under 0.64.0 against
`} // Invalid characters` under 0.67.0, which is what rsvelte already printed. Measured on the
bump arm over the oracle's own 33,838-component population: 0 new failures, 2 retired. The same
...
`lsp-known-failures.json` contains 23790 entries. Fixture and upstream entries identify one normalized
...
Partition of `lsp-known-failures.json` by key kind: `21792 + 1684 + 314` — real-world corpus
...
Partition of `lsp-known-failures.json` by request phase: `11900 + 11890`
...
Ratchet: `parse-ast-known-failures.json`, currently **139 entries**.
...
Partition of `parse-ast-known-failures.json` by cluster: `51 + 28 + 24 + 12 + 8 + 5 + 4 + 4 + 2 + 1`
...
| `unclustered` | 24 | 15 | keys nobody has classified. The cluster exists so an unclassified key reads as unclassified instead of joining someone else's row. `ExpressionStatement.directive` left in #4133: OXC lifts a `<script>`'s directive prologue out of `Program::body` into `Program::directives`, ESTree keeps it in `body`, and the script-program converter read only `body`. |
...
`legacy`, so 139 keys are **79 distinct bases**: 60 appear on both axes and 19 on one
(60x2 + 19 = 139, a 1.76x collapse), and 79 is therefore the defect ceiling. The per-cluster
collapse is not uniform — `estree-fields`, `comment-attachment`, `loc-presence` and `node-type`
are 2.00x (every base is on both axes), `span` 1.89x, `unclustered` 1.60x, `css-shape` 1.50x,
