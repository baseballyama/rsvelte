...
**Current baseline: `fmt-known-failures.json`, 523 entries.** The 789-entry
...
Partition of `fmt-known-failures.json` by cluster: `244 + 214 + 15 + 36 + 12 + 1 + 1`

**The partition is now the mechanical rule applied to all 523 entries**, where it
...
| 518 | no upstream report and no pinned deliberate divergence; elimination is the only end state open to these entries | none |

Partition of `fmt-known-failures.json` by mechanism: `1 + 2 + 1 + 1 + 518`
...
**What this population is not.** `fmt-known-failures.json` holds 523 entries; the 72 carriers
...
Partition of `fmt-known-failures.json` by diff shape: `244 + 127 + 125 + 27`
