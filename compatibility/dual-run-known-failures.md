...
`(fixture, pass)` pairs whose two sides survive `oxc_codegen` normalisation still
...
The comparison is `oxc_codegen(parse(x))` on each side, so anything that round-trip
