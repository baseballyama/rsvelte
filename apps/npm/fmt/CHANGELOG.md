...
- 603b2cc: fix(fmt): a `{@keyword …}` tag glued behind siblings is measured from their end, not from column 0
...
- acfca99: Keep a `{@const}` whose body ends in a `//` comment parseable. The trailing comment
...
- 5eb96e2: Honour `singleAttributePerLine` in the two tag printers that ignored it. An element
...
- 902bbb1: Break the hugged open tag of an attribute-free child element inside `<pre>` /
...
- 468480a: Fix two block-header formatting divergences from the oracle. A call with a spread final
...
- 46838e4: Fix `<!-- prettier-ignore -->` being ignored by the `<pre>`/`<textarea>` sub-format's
...
- b33ddfa: Keep prose fill running across `<!-- prettier-ignore -->`'d inline nodes. An ignore comment
...
- 612b54e: fix(fmt): oracle-match a wrapping empty/whitespace block-display element (#1721)
