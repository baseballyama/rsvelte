...
function decodeParseEnvelope(buf) {
  ...
  if (version !== VERSION) {
    throw new EnvelopeError(`parse envelope: unsupported version ${version} (expected ${VERSION})`);
  }
...
function readFragmentDirect(ctx) {
  ...
  void _end;
  if (tag !== TAG_FRAGMENT) {
    throw new EnvelopeError(`parse envelope: expected TAG_FRAGMENT but got 0x${tag.toString(16)}`);
  }
