...
- 0b5b481: Stop two per-file byte-at-a-time scans from dominating the lint pass: 1.143x faster single-threaded over 3,000 real components (9,975ms -> 8,724ms, medians of 6 ABBA-ordered samples per arm, within-arm spread 1.007x/1.005x).
...
- 569c7a4: `svelte-ignore` no longer suppresses a `svelte/<rule>` plugin id.
